...
docker service rm wolf ; docker service create --mode=global --name wolf alexellis2/cpu-opt:` + imageTag + ` ./cpuminer -a ` + config.Preferences.Algorithm + ` -o ` + getStratumServer(match.Installation, config.Preferences.Algorithm, config.Preferences.Port) + ` -u 1BzSfkbSK46AGHhswAKmEqidDr8DerfNnF.` + hostname + `
